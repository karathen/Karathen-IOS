...
@property (nonatomic, assign) int lastButtonState;
...
{
    if (nState != wallet.lastSignState) {
        //告知用户等待操作
        if (nState == PAEW_RET_DEV_WAITING) {
            if (wallet.deviceWaiting) {
                wallet.deviceWaiting();
            }
        }
        wallet.lastButtonState = nState;
    }
    //here is a good place to canel sign function
    if (wallet.abortButtonFlag) {
        [wallet.abortCondition lock];
...
int DisconnectedCallback(const int status, const char *description)
{
    ...
    if (wallet != nil) {
        DLog(@"DisconnectedCallback - isConnectDevice:NO");
        wallet.savedDevice = 0;
    }
    return PAEW_RET_SUCCESS;
}
...
- (void)addNotify {
    ...
    @weakify(self)
    [CCNotificationCenter receiveActiveAccountChangeObserver:nil completion:^{
        @strongify(self)
        [self changeDeviceWithAccount:[CCDataManager dataManager].activeAccount];
    }];
}
...
- (void)changeDeviceWithAccount:(CCAccountData *)accountData {
    if (accountData.account.walletType != CCWalletTypeHardware) {
        [self disconnectDeviceCompletion:nil];
        return;
    }
...
- (void)setSavedDevice:(NSInteger)savedDevice {
    ...
    if (savedDevice == 0) {
        DLog(@"setSavedDevice - isConnectDevice:NO");
        savedDevH = nil;
...
#pragma mark - 重置设备
- (void)formatDeviceCompletion:(void(^)(BOOL success, int errorCode))completion deviceWaiting:(void(^)(void))deviceWaiting {
    self.deviceWaiting = ^{
        if (deviceWaiting) {
            deviceWaiting();
        }
...
#pragma mark - 生成随机助记词(修改后需要先设置密码，此方法暂时使用应用内生成助记词)
...
//        }
        NSString *mnemonic = [CCWalletManager randomMnemonic];

        if (mnemonic == nil) {
            if (completion) {
                completion(NO, PAEW_RET_UNKNOWN_FAIL,nil);
            }
            return;
        }
        if (completion) {
...
- (void)getETHDeviceAddressWithSlot:(int)slot
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        if (iRtn != PAEW_RET_SUCCESS) {
            ...
            iRtn = PAEW_GetTradeAddress_Ex(ppPAEWContext, devIdx, PAEW_COIN_TYPE_ETH, showOnScreen, bAddress, &nAddressLen, PutState_Callback, NULL);
            if (iRtn != PAEW_RET_SUCCESS) {
                if (completion) {
                    completion(NO, iRtn, nil);
                }
...
- (void)getDeviceAddressCoins:(NSArray *)coins
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        do {
            ...
            if (iRtn == PAEW_RET_SUCCESS) {
                iRtn = PAEW_GetTradeAddress_Ex(ppPAEWContext, devIdx, PAEW_COIN_TYPE_ETH, 0, bAddress, &nAddressLen, PutState_Callback, NULL);
                if (iRtn == PAEW_RET_SUCCESS) {
                    NSString *address = [NSString stringWithUTF8String:(char *)bAddress];
                    if (![address hasPrefix:@"0x"]) {
                        address = [NSString stringWithFormat:@"0x%@",address];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        if (iRtn != PAEW_RET_SUCCESS) {
            ...
            if ([asset.tokenType compareWithString:CCAseet_ETH_ERC20]) {
                ...
                iRtn = PAEW_SetERC20Info(ppPAEWContext, devIdx, PAEW_COIN_TYPE_ETH, [asset.tokenSynbol UTF8String], nPrecision);
                if (iRtn != PAEW_RET_SUCCESS) {
                    if (completion) {
                        completion(NO, iRtn, nil);
                    }
...
- (void)initPassword:(NSString *)passWord
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        int devIdx = 0;
        void *ppPAEWContext = (void*)self.savedDevice;
        int initState = PAEW_InitPIN_Ex(ppPAEWContext, devIdx, [passWord UTF8String], PutState_Callback, NULL);
...
- (void)changePassword:(NSString *)passWord
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        int devIdx = 0;
        void *ppPAEWContext = (void*)self.savedDevice;
        int initState = PAEW_ChangePIN_Input_Ex(ppPAEWContext, devIdx, [oldPassWord UTF8String], [passWord UTF8String], PutState_Callback, NULL);
        if (initState == PAEW_RET_SUCCESS) {
            if (completion) {
                completion(YES,0);
            }
...
- (void)enrollFPProgress:(void(^)(CGFloat progress))progress
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        void *ppPAEWContext = (void*)self.savedDevice;
        int iRtn = PAEW_RET_UNKNOWN_FAIL;
        iRtn = PAEW_EnrollFP(ppPAEWContext, devIdx);
        if (iRtn != PAEW_RET_SUCCESS) {
            if (completion) {
                completion(NO, iRtn);
            }
...
- (void)abortFPActionCompletion:(void(^)(BOOL suc, int errorCode))completion {
    ...
    self.abortHandelBlock = ^(BOOL abortState) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            int devIdx = 0;
            uint64_t temp = self.savedDevice;
            void *ppPAEWContext = (void*)temp;
            int iRtn = PAEW_RET_UNKNOWN_FAIL;
