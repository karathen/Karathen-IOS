...
- (void)importHardwareByMnemonic {
    ...
    [[CCHardwareWallet hardwareWallet] formatDeviceCompletion:^(BOOL success, int errorCode) {
        if (success) {
            //设置密码
            [[CCHardwareWallet hardwareWallet] initPassword:pwd completion:^(BOOL suc, int errorCode) {
                if (success) {
                    //写入助记词
...
- (void)importHardwareFail {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.alertView hiddenView];
        [MBProgressHUD showMessage:Localized(@"Hardware wallet import failed")];
    });
}
