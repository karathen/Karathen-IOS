...
- (NSArray *)toMnemonicArray:(NSData *)data {
    ...
    NSMutableArray *a =
    CFBridgingRelease(CFArrayCreateMutable(SecureAlloc(), data.length * 3 / 4, &kCFTypeArrayCallBacks));
...
- (NSString *)toMnemonic:(NSData *)data {
    ...
    if (a != nil) {
        return CFBridgingRelease(CFStringCreateByCombiningStrings(SecureAlloc(), (__bridge CFArrayRef) a, CFSTR(" ")));
    } else {
        return nil;
    }
}
...
- (NSString *)normalizeCode:(NSString *)code {
    NSMutableString *s = CFBridgingRelease(CFStringCreateMutableCopy(SecureAlloc(), 0, (__bridge CFStringRef) code));
...
- (NSData *)toEntropy:(NSString *)code {
    NSArray *words = wordList;
    NSArray *a = CFBridgingRelease(CFStringCreateArrayBySeparatingStrings(SecureAlloc(), (__bridge CFStringRef) [self normalizeCode:code], CFSTR(" ")));
...
- (NSData *)toSeed:(NSArray *)code withPassphrase:(NSString *)passphrase {
    // Prepare data
    NSString *pass = CFBridgingRelease(CFStringCreateByCombiningStrings(SecureAlloc(), (__bridge CFArrayRef) code, CFSTR(" ")));
